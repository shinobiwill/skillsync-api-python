...
class Settings(BaseSettings):
    ...
    AZURE_CONTAINER_NAME: str = config(
        "AZURE_CONTAINER_NAME", default="skillsync-files"
    )
    ...
    AZURE_TEXT_ANALYTICS_ENDPOINT: str = config(
        "AZURE_TEXT_ANALYTICS_ENDPOINT", default=""
    )
    ...
    MAX_FILE_SIZE: int = config(
        "MAX_FILE_SIZE", default=50 * 1024 * 1024, cast=int
    )  # 50MB
    ...
    RATE_LIMIT_WINDOW: int = config(
        "RATE_LIMIT_WINDOW", default=3600, cast=int
    )  # 1 hora
