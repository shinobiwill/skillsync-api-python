...
class ResumeSearchRequest(BaseModel):
    ...
    sort_by: str = Field(
        default="created_at", pattern="^(created_at|updated_at|title|match_score)$"
    )
...
class AnalysisSearchRequest(BaseModel):
    ...
    created_before: Optional[datetime] = None
    sort_by: str = Field(
        default="created_at", pattern="^(created_at|match_score|processing_time)$"
    )
    sort_order: str = Field(default="desc", pattern="^(asc|desc)$")
    page: int = Field(default=1, ge=1)
    page_size: int = Field(default=20, ge=1, le=100)
...
class FeedbackCreateRequest(BaseModel):
    ...
    type: str = Field(
        ..., pattern="^(analysis_feedback|feature_request|bug_report|general)$"
    )
    title: str = Field(..., min_length=1, max_length=255)
