...
from app.data.mongo_repository import (
    AnalysisMongoRepository,
...
class AnalysisService:
    ...
    async def create_analysis(
        self, user_id: UUID, request: AnalysisCreateRequest
    ) -> AnalysisResponse:
        ...
        try:
            ...
            await self._process_analysis_async(
                created_analysis, request.job_description
            )
    ...
    async def _process_analysis_async(
        self, analysis: CompatibilityAnalysis, job_description: Optional[str] = None
    ) -> None:
        ...
        try:
            ...
            await self.analysis_repo.update_analysis_status(
                analysis.analysis_id, AnalysisStatus.PROCESSING.value
            )

            # Obter conteúdo do currículo
            resume_content = await self._get_resume_content(analysis.resume_id)
            if not resume_content:
                await self._handle_analysis_error(
                    analysis.analysis_id, "Failed to extract resume content"
                )
                return
            ...
            if not job_content:
                await self._handle_analysis_error(
                    analysis.analysis_id, "Failed to get job description"
                )
                return
            ...
            if cached_result:
                ...
                # Processar com IA
                detailed_analysis = await self._analyze_with_ai(
                    resume_content, job_content
                )

                # Armazenar em cache
                await self.cache_repo.cache_analysis(
                    cache_key, detailed_analysis, ttl_hours=24
                )
            ...
            detailed_analysis["jobId"] = (
                str(analysis.job_id) if analysis.job_id else None
            )
            ...
            await self.analysis_repo.execute_query(
                ...
                """,
                {
                    ...
                    "match_score": detailed_analysis["compatibilityReport"][
                        "overallScore"
                    ],
    ...
    async def _get_resume_content(self, resume_id: UUID) -> Optional[str]:
        ...
        try:
            ...
            content = await self.file_service.extract_text_from_file(
                resume.data_lake_file_id
            )
            return content
    ...
    async def _get_job_content(
        self, job_id: Optional[UUID], job_description: Optional[str]
    ) -> Optional[str]:
        """Obter descrição da vaga"""
        try:
            if job_description:
    ...
    async def _analyze_with_ai(
        self, resume_content: str, job_content: str
    ) -> Dict[str, Any]:
    ...
    def _convert_mongo_to_response(
        self, mongo_data: Dict[str, Any], sql_data: Dict[str, Any]
        ...
        try:
            return DetailedAnalysisResponse(
                ...
                extracted_skills=mongo_data.get("resumeAnalysis", {}).get(
                    "extractedSkills", []
                ),
                experience_matches=mongo_data.get("resumeAnalysis", {}).get(
                    "experience", []
                ),
                education=mongo_data.get("resumeAnalysis", {}).get("education", []),
                languages=mongo_data.get("resumeAnalysis", {}).get("languages", []),
                certifications=mongo_data.get("resumeAnalysis", {}).get(
                    "certifications", []
                ),
                compatibility_scores=mongo_data.get("compatibilityReport", {}).get(
                    "categoryScores", {}
                ),
                strengths=mongo_data.get("compatibilityReport", {}).get(
                    "strengths", []
                ),
                weaknesses=mongo_data.get("compatibilityReport", {}).get(
                    "weaknesses", []
                ),
                recommendations=mongo_data.get("compatibilityReport", {}).get(
                    "recommendations", []
                ),
                improvement_areas=mongo_data.get("compatibilityReport", {}).get(
                    "improvementAreas", []
                ),
