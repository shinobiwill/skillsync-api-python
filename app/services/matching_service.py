...
class MatchingService:
    ...
    def _extract_skills(self, text: str, tags_json: str = None) -> Set[str]:
        ...
        common_skills = [
            'python', 'java', 'javascript', 'typescript', 'nodejs', 'react', 
            'angular', 'vue', 'fastapi', 'django', 'flask', 'spring', 
            'docker', 'kubernetes', 'aws', 'azure', 'gcp', 'git', 'sql',
            'postgresql', 'mysql', 'mongodb', 'redis', 'elasticsearch',
            'linux', 'windows', 'ci/cd', 'devops', 'agile', 'scrum',
            'rest api', 'graphql', 'microservices', 'machine learning',
            'data science', 'cybersecurity', 'pentesting', 'siem', 'qradar',
            'wireshark', 'nmap', 'nessus', 'iso 27001', 'lgpd', 'gdpr',
    ...
    def _identify_strengths(
        ...
        if experience_score >= 0.7:
            strengths.append(
                "Experiência profissional adequada para a vaga"
            )
        
        if matched_skills:
            top_skills = matched_skills[:3]
            strengths.append(
                f"Domínio em: {', '.join(top_skills)}"
            )
    ...
    def _identify_weaknesses(
        ...
        if skills_score < 0.5 and missing_skills:
            top_missing = missing_skills[:3]
            weaknesses.append(
                f"Gap de habilidades: {', '.join(top_missing)}"
            )
        
        if experience_score < 0.5:
            weaknesses.append(
                "Experiência profissional abaixo do esperado"
            )
        
        return weaknesses if weaknesses else ["Nenhuma fraqueza significativa identificada"]
    ...
    async def calculate_match(
        ...
        resume_text = f"{resume.resume_title} {resume_version.summary if resume_version else ''}"
        resume_skills = self._extract_skills(
            resume_text,
            resume_version.tags if resume_version else None
        )
        ...
        for resume in resumes:
            try:
                ...
                if match_result.overall_score >= min_score:
                    ...
                    matches.append(ResumeMatchItem(
                        ...
                        summary=version.summary[:200] + "..." if version and version.summary else None
                    ))
