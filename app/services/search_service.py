...
class SearchService:
    ...
    def _generate_highlights(
        self, text: str, keywords: List[str], max_highlights: int = 3
    ) -> List[str]:
        if not text or not keywords:
            return []
    ...
    async def search_resumes(
        self, search_request: SearchResumeRequest
    ) -> SearchResumeResponse:
        ...
        query = select(Resume).join(
            ResumeVersion, Resume.resume_id == ResumeVersion.resume_id
        )
        ...
        if search_request.skills:
            for skill in search_request.skills:
                query = query.where(ResumeVersion.tags.ilike(f"%{skill}%"))
        ...
        for resume in resumes:
            ...
            search_results.append(
                SearchResultItem(
                    ...
                    summary=(
                        latest_version.summary[:200] + "..."
                        if latest_version and latest_version.summary
                        else ""
                    ),
                    score=score,
    ...
    async def search_jobs(self, search_request: SearchJobRequest) -> SearchJobResponse:
        start_time = time.time()
        ...
        if search_request.location:
            query = query.where(Job.location.ilike(f"%{search_request.location}%"))
